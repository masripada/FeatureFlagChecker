...
<body>
  ...
  <script>
    const CODE = '${code}';
    const FUNCTION_URL = '/api/HttpTrigger';

    // Clean the URL bar — removes ?code=... from the browser address bar
    ...
    async function checkFlag() {
      ...
      out.innerHTML = '';
      try {
        const res  = await fetch(FUNCTION_URL + '?code=' + CODE + '&feature=' + featureId);
        ...
        out.innerHTML = \`<div class="result error">
          <div class="result-badge"><span class="result-icon">\u26A0\uFE0F</span><div><div class="result-name">Error</div><div class="result-status">unreachable</div></div></div>
